...
DORA metrics (deployment frequency, lead time for changes, change failure rate, time to restore service) are a starting point. But Agentic AI may change what these metrics mean: deployment frequency may increase dramatically while change failure rate also increases, because more experiments are being run. Whether that is good or bad depends on the organisation's risk appetite and feedback loop speed.

The deeper measurement question: are we measuring agent productivity or team capability? These are different things. A team that produces more code with agents but understands less of their system is less capable, even if the output metrics look good.

Measurement also needs to address the team level explicitly, not just the individual and organisational levels. DORA metrics are already team metrics, which is why they are useful here. But other team-level signals matter too: cognitive load (is it shifting, not just falling?), psychological safety (can the team surface agent failures without blame?), and the health of shared team artefacts (are specs accumulating knowledge or decaying?). Team-level measurement is where the framework's diagnostic value is often strongest, because most dysfunction presents at the team level first.

**What not to measure.** Lines of code produced, number of commits, or any volume-based metric. These are vanity metrics in any context and become actively misleading in an agentic context where an agent can produce thousands of lines of code in minutes. Volume is not value.
